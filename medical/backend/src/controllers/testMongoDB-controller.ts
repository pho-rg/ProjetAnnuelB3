...
const nouvelUtilisateurPOST = async (
  ...
  try {
    ...
    return response
      .status(201)
      .send({
        message: 'Un nouvel utilisateur prédéfini a été ajouté avec succès !',
      });
  } catch (error) {
    console.log(error);
  }
...
const utilisateurPATCH = async (
  ...
  try {
    ...
    User.updateOne({ _id: body.id }, new User())
      .exec()
      .then((result) => {
        console.log(result);
        return response.status(200).send('result');
...
export {
  utilisateurGET,
  nouvelUtilisateurPOST,
  utilisateurUniqueGETONE,
  utilisateurPATCH,
  utilisateurDELETEONE,
};
